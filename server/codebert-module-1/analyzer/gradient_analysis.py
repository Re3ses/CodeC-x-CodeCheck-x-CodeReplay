...
class GradientAnalysis:
    ...
    def analyze_embedding_gradients(self, code1: str, code2: str) -> dict:
        ...
        try:
            ...
            attribution_scores = (torch.abs(grad1[0]) + torch.abs(grad2[0])).cpu().numpy()
            ...
            ax1.plot(x, attribution_scores, color='#1f77b4', linewidth=1, alpha=0.5, label='Attribution')
            ax1.plot(top_dims, attribution_scores[top_dims], 'ro', label='Top Contributors', markersize=6)
            ...
            num_bins = len(attribution_scores) // bin_size + (1 if len(attribution_scores) % bin_size > 0 else 0)
            ...
            im = ax2.imshow(binned_scores_2d, aspect='auto', cmap='viridis', interpolation='nearest')
            
            # Add a colorbar with better positioning - increase the pad to move it down
            cbar = plt.colorbar(im, ax=ax2, orientation='horizontal', pad=0.35, aspect=40)
            ...
            ax2.set_xticklabels([f"{i*bin_size}" for i in range(0, num_bins, max(1, num_bins // 10))], fontsize=9)
            ...
            ax2.set_title('Attribution Strength by Dimension Groups', fontsize=12, pad=10)
            ...
            for dim, score in zip(top_dims, top_scores):
                context = self.get_dimension_context(code1, int(dim), attribution_scores)
            ...
            plt.text(0.5, 0.5, f"Error during analysis:\n{str(e)}", 
    ...
    def get_dimension_context(self, code: str, dimension: int, attribution_scores: np.ndarray) -> dict:
        ...
        try:
            ...
            print(f"Found {len(relevant_tokens)} relevant tokens for dimension {dimension}")
