...
class CodeBERTAnalyzer:
    ...
    def preprocess_code(self, code: str) -> str:
        ...
        code = re.sub(r'\b(let|var|const)\s+(\w+)', 
                     lambda m: f"{m.group(1)} {re.sub(r'([A-Z])', lambda x: x.group(1).lower(), m.group(2))}", 
                     code)
        
        # Normalize strings
        code = re.sub(r'(["\'])(.*?)\1', lambda m: f"'{m.group(2).replace("'", "\\'")}'", code)
    ...
    def compute_similarity_matrix(self, snippets: List[SnippetInfo]) -> Tuple[List[List[float]], List[Dict]]:
        ...
        for i in range(n):
            for j in range(i, n):
                if i == j:
                    ...
                    similarity = round(self.calculate_similarity(codes[i], codes[j]) * 100)
    ...
    def compute_sequential_similarities(self, snapshots: List[Dict]) -> List[SequentialSimilarity]:
        ...
        for i in range(len(snapshots) - 1):
            ...
            similarities.append(SequentialSimilarity(
                from_index=i,
                to_index=i + 1,
