...
interface AttentionResponse {
    ...
    similarities: {
        tokens: string[];
        layer_wise_attention: number[][][][]; // [num_layers][batch_size][num_heads][seq_len][seq_len]
    };
    error?: string;
}
...
const AttentionView: React.FC<AttentionProps> = ({ codeSnippets }) => {
    ...
    // Update number of layers and heads when attention data is available
    useEffect(() => {
        if (attentionData.length > 0) {
            setNumLayers(attentionData.length);
            // Assuming the number of heads is the same across all layers and batches
            if (attentionData[0] && attentionData[0][0]) {
                setNumHeads(attentionData[0][0].length);
            } else {
                setNumHeads(0);
            }
            setSelectedLayer(0); // Reset selected layer on new data
            setSelectedHead(0);   // Reset selected head on new data
        }
    }, [attentionData]);
    ...
    // Process code to create highlighted spans based on attention weights
    const processCodeWithAttention = (code: string, isCode1: boolean): JSX.Element[] => {
        if (!tokens.length || !attentionData.length || numLayers === 0 || numHeads === 0) {
            return [<pre key="raw" className="text-gray-300">{code}</pre>];
        }

        const sepIndex = findSepIndex();
        const currentAttentionLayer = attentionData[selectedLayer]; // Get the selected layer
        const firstBatchAttention = currentAttentionLayer[0]; // Assumes a single batch
        ...
        return codeTokens.map((token, idx) => {
            ...
            // Find the attention weight for this token to all tokens in the other snippet for the selected head
            let maxAttention = 0;
            ...
            tokenIndices.forEach(tokenIndex => {
                ...
                otherIndices.forEach(otherIndex => {
                    const attentionValue = firstBatchAttention[selectedHead][tokenIndex][otherIndex];
                    if (attentionValue > maxAttention) {
                        maxAttention = attentionValue;
                        bestMatch = { index: otherIndex, attention: attentionValue };
                    }
                });
            });
        ...
        <div className="bg-gray-900 rounded-lg shadow-lg overflow-hidden">
            ...
            {attentionData.length > 0 && (
                <div className="flex items-center space-x-4 p-4 border-b border-gray-700">
                    <div>
                        <label htmlFor="layer" className="block text-sm font-medium text-gray-300">Layer</label>
                        <select
                            id="layer"
                            className="mt-1 block w-full py-2 px-3 border border-gray-700 bg-gray-800 text-gray-200 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                            value={selectedLayer}
                    ...
                    <div>
                        <label htmlFor="head" className="block text-sm font-medium text-gray-300">Head</label>
                        <select
                            id="head"
                            className="mt-1 block w-full py-2 px-3 border border-gray-700 bg-gray-800 text-gray-200 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                            value={selectedHead}
