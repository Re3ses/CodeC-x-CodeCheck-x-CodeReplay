...
export default function Page() {
     ...
     */
    async function getToken(input = "", expected = null) {
        const data = {
     ...
     */
    async function handleSubmit(isTest = true) {
        ...
        const tokens = submissions?.map(token => {
            return token.token
        })
        ...
        setTimeout(async () => {
            try {
                ...
                const accepted = batchSubmissions.submissions.filter(
                    obj => obj.status.description === "Accepted"
                )
                const score = {
    ...
    async function sendLiveCodeRequest(event) {
            ...
            <Panel className="min-w-[20em] overflow-scroll flex flex-col gap-2">
                    ...
                    <Dialog>
                        ...
                        <DialogContent>
                            <DialogHeader>
                                ...
                                <DialogDescription>
                                    Notify the creator of this problem for a live coding session,
                                    you will recieve a "Join" button should they accept.
                                </DialogDescription>
                            </DialogHeader>
                ...
                <div className="p-3">
                    ...
                    <table className="w-full">
                        ...
                        {problem?.test_cases.map((val, index) => {
                            if (val.is_sample) {
                                return (
                                    <tr className={index % 2 == 0 ? "" : "bg-zinc-900"}>
                                        <td>{val.input}</td>
                                        <td>{val.output}</td>
                                    </tr>
                                )
